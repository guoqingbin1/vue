  ...
  <div>
    ...
    <el-form :inline="true" :model="formInline" class="user-search">
      <el-form-item label="搜索：">
        <el-select size="small" v-model="formInline.userStatus" placeholder="请选择">
      ...
      <el-form-item label="">
        <el-input size="small" length="8" v-model="formInline.loginName" placeholder="输入用户名"></el-input>
      </el-form-item>
      <el-form-item label="">
        <el-input size="small" length="11" v-model="formInline.mobile" placeholder="输入手机号"></el-input>
      </el-form-item>
      <el-form-item>
    ...
    <el-table size="small" @selection-change="selectChange" :data="userData" highlight-current-row v-loading="loading" border element-loading-text="拼命加载中" style="width: 100%;">
      ...
      <el-table-column align="center" sortable prop="loginName" label="用户名" width="120">
      </el-table-column>
      ...
      <el-table-column align="center" sortable prop="registerIp" label="ip" width="120">
      </el-table-column>
      <el-table-column align="center" sortable prop="updatedTime" label="修改时间" min-width="120">
        <template slot-scope="scope">
          <div>{{scope.row.updatedTime|timestampToTime}}</div>
        </template>
      </el-table-column>
      <el-table-column align="center" sortable prop="userStatus" label="状态" min-width="50">
        <template slot-scope="scope">
          <el-switch v-model="scope.row.userStatus=='1'?nshow:fshow" active-color="#13ce66" inactive-color="#ff4949" @change="editType(scope.$index, scope.row)">
          </el-switch>
        </template>
      </el-table-column>
  ...
  methods: {
        ...
        if (res!=null && res.code != 200) {
          this.$message({
            ...
            // 分页赋值
             ...
             this.pageparm.total = res.data.page.totalCount
    ...
    handleEdit: function(index, row) {
      ...
      if (row != undefined && row != "undefined") {
        ...
        this.editForm.loginName = decodeURI(row.loginName);
        this.editForm.mobile = row.mobile;
        this.editForm.email = decodeURI(row.email);
        this.editForm.addUser = false;
      } else {
        this.title = "添加用户";
    ...
    submitForm(editData) {
      this.$refs[editData].validate(valid => {
        if (valid) {
          // 请求方法
        //  this.editForm.loginName = this.editForm.loginName);
