      ...
      <el-row>
        <el-col :span="12">
          <el-form-item prop="code">
            <el-input type="text" v-model="ruleForm.imgCode" auto-complete="off" placeholder="图形验证码" @keyup.enter.native="submitForm('ruleForm')"></el-input>
          </el-form-item>
        </el-col>
  ...
  methods: {
    ...
    getuserpwd() {
      if (getCookie("user") != "" && getCookie("pwd") != "") {
        ...
        this.ruleForm.userPwd = getCookie("pwd");
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.logining = true;
          // 测试通道，不为空直接登录
          // localStorage.setItem('logintoken', "1461346131dfsdjl223")
          // setTimeout(() => {
          //  this.logining = false
